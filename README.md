...
- **按类型推荐**（本项目新增）：多选类型（最多 3 个）→ 走 TMDB `discover` 接口做多类型 **AND 过滤**（同时满足全部所选类型）→ 按评分人数排序 → 无限滚动加载
...
- 多类型走 TMDB `discover` 接口的 `with_genres`，多个 id 用 **`,` 连接表示 AND**（`|` 是 OR）——即"选动作+喜剧"只会推同时属于这两类的影片
