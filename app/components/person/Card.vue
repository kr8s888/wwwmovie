...
<template>
  <NuxtLink :to="`/person/${item.id}`" data-testid="person-link">
    <div block bg-gray4:10 p1 class="aspect-10/16" data-testid="person-container">
      <NuxtImg v-if="item.profile_path" width="300" height="450" format="webp" :src="`/tmdb${item.profile_path}`" :alt="item.name" w-full h-full object-cover data-testid="person-image" />
